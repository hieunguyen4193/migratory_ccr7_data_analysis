...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
p <- DimPlot(object = s.obj, reduction = "RNA_UMAP", label = TRUE, label.box = TRUE, repel = TRUE, group.by = "State")
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
p <- FeaturePlot(object = s.obj, reduction = "RNA_UMAP", label = TRUE, features = c("Pseudotime"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
for (tmp.col in colnames(cellWeights)){
  p <- DimPlot(object = s.obj, reduction = "RNA_UMAP", group.by = tmp.col, label = TRUE, pt.size = 1) + scale_color_manual(values = c("gray", "red"))
  print(p)
}
```
