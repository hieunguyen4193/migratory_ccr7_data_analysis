...
generate_vlnplot <- function(obj, 
  ...
  plot_list <- purrr::map(features, function(x) modify_vlnplot_from_seurat(obj = obj,feature = x, cluster.order = cluster.order, colors = colors, plot.for.CCR7 = plot.for.CCR7))
...
##### stacked violin plot
# features <- sample(row.names(s.obj), 12) ### > gene list
...
##### heatmap of selected genes in all clusters
# cluster.markers <- readRDS("/media/hieunguyen/HD0/outdir/CRC1382/FHager_datasets/20230513/220907_FH_v0.1/data_analysis/01_output/cluster_markers.rds")
# selected.genes <- c()
# for (cluster.id in unique(cluster.markers$cluster)){
#   tmp.selected.genes <- subset(cluster.markers, cluster.markers$cluster == cluster.id) %>% arrange(desc(avg_log2FC)) %>% head(10) %>% pull(gene)
#   selected.genes <- c(selected.genes, tmp.selected.genes)
# }
# DoHeatmap(object = s.obj, features = selected.genes, assay = "RNA", draw.lines = TRUE) # <<< 

##### re-order Ccr7 clusters
s.obj$ccr7_cluster <- factor(s.obj$ccr7_cluster, levels = c("CCR7_cluster1", "CCR7_cluster0", "others"))

# p <- VlnPlot(object = s.obj, features = c("Ccr7"), group.by = "ccr7_cluster", cols = c("red", "blue", "gray"), pt.size = 1)

p <- VlnPlot(object = subset(s.obj, ccr7_cluster != "others"), features = c("Ccr7"), group.by = "ccr7_cluster", cols = c("red", "blue"), pt.size = 1)

ggsave(plot = p, filename = "test_CCR7.svg", path = path.to.save.output, device = "svg", width = 4, height = 10, dpi = 300)

##### stacked violin plot for CCR7 clusters
p <- generate_vlnplot(obj = subset(s.obj, ccr7_cluster != "others"), 
                      features = features, 
                      pt.size = 0, cluster.order = NULL, 
                      colors = c("red", "blue"), plot.for.CCR7 = TRUE)
# hue_pal()(12)
