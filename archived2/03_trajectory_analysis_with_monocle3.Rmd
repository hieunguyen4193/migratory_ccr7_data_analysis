...
if (file.exists(file.path(path.to.03.output, "monocle3.rds")) == FALSE){
  #####----------------------------------------------------------------------#####
  ...
  saveRDS(monocle.obj, file.path(path.to.03.output, "monocle3.rds"))
} else {
  monocle.obj <- readRDS(file.path(path.to.03.output, "monocle3.rds"))
}
...
for (principal_node in list_of_principal_points){
  ...
  p <- FeaturePlot(object = s.obj, reduction = "RNA_UMAP", features = sprintf("pseudotime_%s", principal_node), label = TRUE, label.size = 8, label.color = "red") + scale_color_viridis_c()
  print(p)
  cat("\n \n")
}
```
...
if (file.exists(file.path(path.to.03.output, "monocle.obj_graph_test_res.rds")) == FALSE){
  monocle.obj_graph_test_res <- graph_test(monocle.obj, neighbor_graph="principal_graph", cores=16)
  saveRDS(monocle.obj_graph_test_res, file.path(path.to.03.output, "monocle.obj_graph_test_res.rds"))
} else {
  monocle.obj_graph_test_res <- readRDS(file.path(path.to.03.output, "monocle.obj_graph_test_res.rds"))
}
...
for (tmp.col in colnames(cellWeights)){
  p <- DimPlot(object = s.obj, reduction = "RNA_UMAP", group.by = tmp.col, label = TRUE, pt.size = 1.5)
  print(p)
}
```
...
if (file.exists(file.path(path.to.03.output, "sce.rds")) == FALSE){
  ...
  #                               plot = TRUE,
  #                               parallel = FALSE)
  # saveRDS(eval_to_choose_K, file.path(path.to.03.output, "eval_to_choose_K.rds"))
  ...
  saveRDS(sce, file.path(path.to.03.output, "sce.rds"))  
} else {
  sce <- readRDS(file.path(path.to.03.output, "sce.rds"))
}
```
...
##### Test within lineage
#####----------------------------------------------------------------------#####
if (file.exists(file.path(path.to.03.output, "tradeseq_test_res.rds")) == FALSE){
  ...
  saveRDS(tradeseq.testres, file.path(path.to.03.output, "tradeseq_test_res.rds"))
} else {
  tradeseq.testres <- readRDS(file.path(path.to.03.output, "tradeseq_test_res.rds"))
}
